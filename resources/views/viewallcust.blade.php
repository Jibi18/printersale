...
<div class="col col-12 col-sm-3 col-md-3 col-lg-3">
  <form action="/viewallcust" method="post">
  {{ csrf_field() }}
  <table class="table">
   <tr>
       <th>NAME</th>
...
@foreach($customers as $newmodel)

   <tr>
       <td>{{ $newmodel->Name }}</td>
       <td>{{ $newmodel->Add  }}</td>
       <td>{{ $newmodel->Email }}</td>
       <td>{{ $newmodel->Phno  }}</td>
       <td><a class ="btn btn-danger" href="/newmodel/{{$newmodel->id}} /delete">DELETE</a></td>
       </tr>
 @endforeach
